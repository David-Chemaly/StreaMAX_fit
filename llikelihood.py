...
def logl(params, dict_data, n_particles=10000, n_min=101, var_ratio=9.0):
    ...
    if jnp.all(jnp.isnan(r_bin)):
        ...
        var_data = dict_data['r_err']**2 + params[13]**2
        var_model = sig_bin**2 / count_bin

        n_uncertain = jnp.sum(var_model >= (var_data / var_ratio))
        if n_uncertain == 0:
            var = var_data + var_model
            logl  = -.5 * jnp.sum(  (r_bin - dict_data['r'])**2 / var  + jnp.log(2 * jnp.pi * var)  )
        else:
            logl = BAD_VAL * n_uncertain / 100
