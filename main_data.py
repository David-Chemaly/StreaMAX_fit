...
def save_best_fit_image(output_dir, path_data, name, dict_data, best_params, n_particles, stream_fn, mode, mass_loss_mode='linear_to_zero'):
    ...
    if mode == 'los_only':
        ...
        theta_lime = np.array([dict_data['theta'][i_vz]])
        r_lime = np.array([r_bin[i_vz]])
    else:
        theta_lime = dict_data['theta']
        r_lime = r_bin
    x_bin = r_lime * np.cos(theta_lime + dict_data['delta_theta'])
    y_bin = r_lime * np.sin(theta_lime + dict_data['delta_theta'])
...
def attach_los_data(dict_data, cfg):
    ...
    dict_data['r_v'] = float(dict_data['r'][i_vz])
